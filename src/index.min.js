...
var FeedType = class {
  ...
  constructor(def) {
    ...
    ["id", "name", "icon", "configuration", "refreshRate", "ephemeral", "idealWidth"].forEach((k2) => {
  ...
  findError(v2) {
    if (this.type === "URL") {
      if (!/^http(?:s)?:\/\//i.test(v2)) return `invalid URL`;
      try {
        new URL(v2);
      } catch (e12) {
        return `invalid URL`;
...
var [$newSpaceDialog, openNewSpace, closeNewSpace] = makeBooleanState();
...
var spacesTauriStore = await load(`spaces.json`, loadOptions);
(await spacesTauriStore.entries()).forEach(([k2, v2]) => $spaces.setKey(k2, v2));
spacesTauriStore.onChange((k2, v2) => $spaces.setKey(k2, v2));
var $currentSpace = map($spaces.get().inbox);
function setCurrentSpace(id3) {
  const spaces = $spaces.get();
  if (spaces[id3]) $currentSpace.set(spaces[id3]);
  else {
...
// src/el/space-picker.js
var import_lit114 = __toESM(require_lib(), 1);
customElements.define("z-space-picker", class extends i4 {
  #spaces = new import_lit114.StoreController(this, $spaces);
  #currentSpace = new import_lit114.StoreController(this, $currentSpace);
  static styles = i`
    :host {
      display: block;
    }
  ...
  render() {
    ...
    let wsList = ws.length ? x`<sl-menu-item disabled>No spaces.</sl-menu-item>` : ws.map(([id3, { name }]) => x`<sl-menu-item value=${id3}>${name}</sl-menu-item>`);
...
// src/el/space-edit-dialog.js
var import_lit118 = __toESM(require_lib(), 1);
customElements.define("z-space-edit-dialog", class extends i4 {
  #open = new import_lit118.StoreController(this, $newSpaceDialog);
  static styles = i``;
  render() {
    const label = "New Space";
    return x`<sl-dialog label=${label} .open=${this.#open.value} @sl-request-close=${closeNewSpace}>
      Lorem ipsum dolor sit amet, consectetur adipiscing elit.
...
customElements.define("z-space", class extends i4 {
...
// src/el/space-tools.js
var import_lit122 = __toESM(require_lib(), 1);
function editSpace() {
}
customElements.define("z-space-tools", class extends i4 {
  #currentSpace = new import_lit122.StoreController(this, $currentSpace);
  static styles = i`
    :host {
      display: block;
  ...
  render() {
    ...
    return x`<sl-dialog label="Add Feed" .open=${this.#open.value} @sl-request-close=${closeAddFeed}>
      <form @submit=${this.handleAddFeed}>
        ...
        >
          <sl-option></sl-option>
          ${Object.values(this.#feedTypes.value || {}).map((ft) => x`<sl-option value=${ft.id}>${ft.name}</sl-option>`)}
        </sl-select>
