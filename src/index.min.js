...
var FeedType = class {
  ...
  constructor(def) {
    ...
    ["name", "icon", "configuration", "refreshRate", "ephemeral", "idealWidth"].forEach((k2) => {
      if (def[k2]) this[k2] = def[k2];
    });
  }
  validate(v2) {
    if (this.type === "URL") {
      if (!/^http(?:s)?:\/\//i.test(v2)) return false;
      try {
        new URL(v2);
      } catch (e12) {
        return false;
...
// src/store.js
var loadOptions = { autoSave: 100 };
function makeBooleanState(def = false) {
  const $state = atom(def);
  const turnOn = (id3) => $state.set(id3 || true);
  const turnOff = (evt) => {
...
var [$settingsDialog, openSettings, closeSettings] = makeBooleanState();
var [$newWorkspaceDialog, openNewWorkspace, closeNewWorkspace] = makeBooleanState();
var [$addFeedDialog, openAddFeed, closeAddFeed] = makeBooleanState();
var $workspaces = map({
  inbox: {
    $id: "inbox",
    icon: "inbox",
...
var workspacesTauriStore = await load(`workspaces.json`, loadOptions);
(await workspacesTauriStore.entries()).forEach(([k2, v2]) => $workspaces.setKey(k2, v2));
workspacesTauriStore.onChange((k2, v2) => $workspaces.setKey(k2, v2));
var $currentWorkspace = map($workspaces.get().inbox);
function setCurrentWorkspace(id3) {
  const workspaces = $workspaces.get();
  if (workspaces[id3]) $currentWorkspace.set(workspaces[id3]);
  else {
...
// src/el/settings-button.js
customElements.define("z-settings-button", class extends i4 {
  static styles = i`
    :host {
      display: block;
    }
  ...
  `;
  render() {
    return x`<sl-icon-button name="gear" label="Settings" @click=${openSettings}></sl-icon-button>`;
  }
});

// src/el/workspace-picker.js
var import_lit114 = __toESM(require_lib(), 1);
customElements.define("z-workspace-picker", class extends i4 {
  #workspaces = new import_lit114.StoreController(this, $workspaces);
  #currentWorkspace = new import_lit114.StoreController(this, $currentWorkspace);
  static styles = i`
    :host {
      display: block;
    }
  ...
  render() {
    ...
    let wsList = ws.length ? x`<sl-menu-item disabled>No workspaces.</sl-menu-item>` : ws.map(([id3, { name }]) => x`<sl-menu-item value=${id3}>${name}</sl-menu-item>`);
    return x`<sl-dropdown @sl-select=${this.handleSelect} value=${curID}>
      <sl-button slot="trigger" caret>
        ${cur?.icon ? x`<sl-icon slot="prefix" name=${cur.icon}></sl-icon>` : E}
...
// src/el/settings-dialog.js
var import_lit116 = __toESM(require_lib(), 1);
customElements.define("z-settings-dialog", class extends i4 {
  #open = new import_lit116.StoreController(this, $settingsDialog);
  static styles = i``;
  render() {
    return x`<sl-dialog label="Settings" .open=${this.#open.value} @sl-request-close=${closeSettings}>
...
// src/el/workspace-edit-dialog.js
var import_lit118 = __toESM(require_lib(), 1);
customElements.define("z-workspace-edit-dialog", class extends i4 {
  #open = new import_lit118.StoreController(this, $newWorkspaceDialog);
  static styles = i``;
  render() {
    const label = "New Workspace";
    return x`<sl-dialog label=${label} .open=${this.#open.value} @sl-request-close=${closeNewWorkspace}>
      Lorem ipsum dolor sit amet, consectetur adipiscing elit.
...
customElements.define("z-workspace", class extends i4 {
...
// src/el/workspace-tools.js
var import_lit122 = __toESM(require_lib(), 1);
function editWorkspace() {
}
customElements.define("z-workspace-tools", class extends i4 {
  #currentWorkspace = new import_lit122.StoreController(this, $currentWorkspace);
  static styles = i`
    :host {
      display: block;
...
// src/el/add-feed-dialog.js
var import_lit124 = __toESM(require_lib(), 1);
customElements.define("z-add-feed-dialog", class extends i4 {
  #open = new import_lit124.StoreController(this, $addFeedDialog);
  #feedTypes = new import_lit124.StoreController(this, $feedTypes);
  static properties = {
  ...
  handleAddFeed(evt) {
    ...
    const result = {};
    conf.forEach((c6) => {
      const fld = form.querySelector(`[name="${c6.name}"]`);
      if (!fld) return console.error(`No field for configuration "${c6.name}".`);
      const value = fld.value;
      if (c6.required && value == null) {
        errorCount++;
      } else if (value != null) {
        if (!type.validate(value)) {
          errorCount++;
        } else {
          result[c6.name] = value;
        }
      }
  ...
  generateFeedConfiguration() {
    if (!this.type) return E;
    const conf = this.#feedTypes.value?.[this.type]?.configuration;
    if (!conf) return E;
    return conf.map((c6) => {
      if (c6.type === "URL") return x`<sl-input
          type="url"
          name=${c6.name}
          label=${c6.label}
          helpText=${c6.helpText}
          placeholder=${c6.placeholder}
          ?required=${c6.required}
        ></sl-input>
      `;
      return E;
    });
  ...
  render() {
    return x`<sl-dialog label="Add Feed" .open=${this.#open.value} @sl-request-close=${closeAddFeed}>
      <form @submit=${this.handleAddFeed}>
        <sl-select
          ...
          hoist
        >${Object.values(this.#feedTypes.value || {}).map((ft) => x`<sl-option value=${ft.name}>${ft.name}</sl-option>`)}</sl-select>
